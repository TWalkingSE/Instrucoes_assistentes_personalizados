...
> **Versão:** 5.0 | **Atualização:** Abril 2026
...
61. Privacidade no Windows (Desktop)
62. Segurança Física e OPSEC Comportamental
63. Segurança Familiar e Proteção de Menores
...
### 21.0 Checklist de Segurança do Roteador (Leigo)

Antes de investir em ferramentas avançadas, todo usuário deve garantir o básico do roteador:

| # | Ação | Prioridade | Como fazer |
|---|---|---|---|
| 1 | **Alterar senha padrão do roteador** | 🔴 Crítico | Acessar 192.168.0.1 ou 192.168.1.1, trocar usuario/senha admin padrão |
| 2 | **Alterar nome da rede (SSID)** | 🟡 Recomendado | Não usar nome que identifique você (ex: "Casa_João") |
| 3 | **Usar WPA3 (ou WPA2-AES no mínimo)** | 🔴 Crítico | Configurações Wi-Fi > Segurança > selecionar WPA3/WPA2-AES |
| 4 | **Desativar WPS** | 🔴 Crítico | WPS tem vulnerabilidade crítica de brute-force |
| 5 | **Criar rede de convidados** | 🟡 Recomendado | Isolar dispositivos de visitantes da rede principal |
| 6 | **Atualizar firmware do roteador** | 🔴 Crítico | Painel admin > Firmware/Update > verificar atualizações |
| 7 | **Desativar administração remota** | 🔴 Crítico | Impede acesso ao painel de fora da rede local |
| 8 | **Configurar DNS seguro** | 🟡 Recomendado | Alterar DNS para Quad9 (9.9.9.9) ou Cloudflare (1.1.1.1) |
| 9 | **Verificar dispositivos conectados** | 🟢 Ideal | Regularmente checar lista de clients no painel admin |
| 10 | **Desativar UPnP** | 🟡 Recomendado | UPnP pode abrir portas automaticamente sem sua permissão |

> ⚠️ **IMPORTANTE:** A maioria dos roteadores domésticos vem com configurações inseguras de fábrica. Completar este checklist leva 15-30 minutos e é a base de toda segurança de rede.

### 21.1 Ferramentas Avançadas
...
### 61.1 Debloat e Anti-Telemetria

| Ferramenta | Open-Source | Tipo | Diferencial | Link |
|---|---|---|---|---|
| **O&O ShutUp10++** | ❌ Não | GUI | Desativa 100+ configurações de telemetria com um clique, recomendações integradas | https://www.oo-software.com/en/shutup10 |
| **WPD (Windows Privacy Dashboard)** | ✅ Sim | GUI | Dashboard visual para desativar telemetria, Cortana, OneDrive | https://wpd.app/ |
| **Privatezilla** | ✅ Sim | GUI | Checklist de privacidade para Windows 10/11 com análise e aplicação | https://github.com/builtbybel/privatezilla |
| **Sophia Script** | ✅ Sim | PowerShell | Script PowerShell para customizar Windows 10/11, remove apps bloat | https://github.com/farag2/Sophia-Script-for-Windows |
| **Chris Titus WinUtil** | ✅ Sim | PowerShell/GUI | Utilitário completo: debloat, tweaks, instalador de programas | https://github.com/ChrisTitusTech/winutil |
| **W10Privacy** | ❌ Não | GUI | Controle granular de telemetria, serviços e tarefas agendadas | https://www.w10privacy.de/english-home/ |
| **Debotnet** | ✅ Sim | GUI | Anti-spy para Windows, scripts modulares | https://github.com/builtbybel/debotnet |
| **WindowsSpyBlocker** | ✅ Sim | Hosts/Firewall | Bloqueia telemetria via hosts file, firewall rules e NCSI | https://github.com/nicehash/WindowsSpyBlocker |

### 61.2 Hardening e Segurança

| Ferramenta | Open-Source | Tipo | Diferencial | Link |
|---|---|---|---|---|
| **HardenTools** | ✅ Sim | GUI | Desativa funcionalidades perigosas (macros, WSH, PowerShell remoting), da Security Without Borders | https://github.com/securitywithoutborders/hardentools |
| **CIS Benchmark Windows** | ❌ Não | Guia/GPO | Baseline de hardening profissional, GPO templates | https://www.cisecurity.org/benchmark/microsoft_windows_desktop |
| **Microsoft Security Compliance Toolkit** | ❌ Não | GPO/LGPO | Ferramentas oficiais MS para aplicar baselines de segurança | https://www.microsoft.com/en-us/download/details.aspx?id=55319 |
| **Windows Defender Application Guard** | ❌ Não | Isolamento | Navegação isolada via Hyper-V (Edge em sandbox) | — |
| **Sandboxie-Plus** | ✅ Sim | Sandbox | Sandbox clássico para isolar apps Windows | https://sandboxie-plus.com/ |
| **Windows Sandbox** | ❌ Não | Sandbox | Sandbox descartável nativo (Win 10/11 Pro) baseado em Hyper-V | — |
| **simplewall** | ✅ Sim | Firewall | Interface simples para WFP, bloqueia conexões por app | https://github.com/henrypp/simplewall |
| **Portmaster** | ✅ Sim | Firewall+DNS | Firewall de aplicação + DNS seguro + privacidade por app | https://safing.io/portmaster/ |

### 61.3 Substituições de Apps Nativos (Privacidade)

| App Nativo | Alternativa Privada | Diferencial |
|---|---|---|
| Windows Search | **Everything** | Busca instantânea, zero telemetria, ultra-leve | https://www.voidtools.com/ |
| Microsoft Edge | **Firefox / LibreWolf / Brave** | Sem telemetria Microsoft, open-source |
| Notepad | **Notepad++ / Kate** | Sem telemetria, extensível | https://notepad-plus-plus.org/ |
| Adobe Reader | **SumatraPDF / Okular** | Sem telemetria, leve | https://www.sumatrapdfreader.org/ |
| Windows Mail | **Thunderbird / Betterbird** | Open-source, PGP nativo | https://www.thunderbird.net/ |
| OneDrive | **Cryptomator + qualquer nuvem** | E2EE sobre qualquer provider |
| Cortana | Desativar | Coleta dados de voz, busca, hábitos |
| Windows Terminal | **Windows Terminal** (manter) | O Terminal moderno é bom; desativar telemetria |
| Fotos | **nomacs / digiKam** | Visualizadores sem telemetria |
| Groove Music | **Strawberry / VLC** | Players open-source sem rastreamento |

> ⚠️ **Dica Windows:** Após uma instalação limpa, execute O&O ShutUp10++ ou Chris Titus WinUtil ANTES de conectar à internet para minimizar dados enviados durante o setup.
...
### 62.1 Proteção Física de Dispositivos

| Produto/Prática | Tipo | Diferencial | Link |
|---|---|---|---|
| **Telas de privacidade (3M, Kensington)** | Filtro físico | Impede visão lateral da tela (shoulder surfing), ângulo de 60° | https://www.3m.com/privacy-screen-protectors |
| **PortaPow USB Data Blocker** | USB blocker | Bloqueia pinos de dados USB, permite apenas carga (anti juice jacking) | https://portapow.com/ |
| **Faraday bags (Mission Darkness, Silent Pocket)** | Bloqueio RF | Bloqueiam celular/GPS/Wi-Fi/BT, impedem rastreamento | https://silentpocket.com/ |
| **Kensington lock** | Trava física | Trava de cabo para laptops em espaços públicos | https://www.kensington.com/ |
| **Webcam covers** | Cobertura física | Coberturas deslizantes para câmeras de laptop | — |
| **YubiKey / chave FIDO2** | Autenticação física | 2FA resistente a phishing, impossível clonar remotamente | https://www.yubico.com/ |
| **Trituradora cross-cut** | Destruição de documentos | Destrói papéis em partículas (não apenas tiras) | — |

### 62.2 Anti-Vigilância e Detecção

| Ferramenta/Prática | Tipo | Diferencial | Link |
|---|---|---|---|
| **AirGuard (Android)** | Anti-tracking BT | Detecta AirTags e rastreadores Bluetooth desconhecidos | https://github.com/nicehash/AirGuard |
| **Apple Tracker Detect (Android)** | Anti-tracking BT | App oficial Apple para detectar AirTags em Android | Play Store |
| **Hidden Camera Detector (apps)** | Detecção | Usa sensor magnético e infravermelho do celular para detectar câmeras | Play Store |
| **RF detector (hardware)** | Detecção RF | Detecta transmissões RF de câmeras/microfones ocultos | — |
| **Faraday tent/room** | Blindagem RF | Para ambientes de máxima segurança (contra emanações EM) | — |

### 62.3 Engenharia Social — Ataques e Defesas

| Ataque | Descrição | Defesa |
|---|---|---|
| **SIM swapping** | Atacante transfere seu número para outro chip | PIN da operadora, migrar para e-SIM, nunca usar SMS como 2FA |
| **Vishing** | Phishing por voz (ligação telefônica) | Nunca fornecer dados por telefone não solicitado; desligue e ligue de volta |
| **Pretexting** | Atacante se passa por suporte técnico, banco, colega | Verificar identidade por canal separado; desconfiar de urgência |
| **Shoulder surfing** | Observação visual de tela/teclado | Tela de privacidade, biometria em público, senhas longas via gerenciador |
| **Dumpster diving** | Busca em lixo por documentos/dispositivos descartados | Trituradora cross-cut, DBAN antes de descartar hardware |
| **Evil maid** | Acesso físico ao dispositivo desatendido | Criptografia de disco + Secure Boot + senha de BIOS/UEFI |
| **Juice jacking** | Carregadores USB públicos que roubam dados | USB data blocker, usar carregador próprio ou tomada |
| **Tailgating** | Seguir funcionário autorizado para área restrita | Não segurar porta para desconhecidos, verificar crachás |

### 62.4 OPSEC Comportamental — Checklist

| # | Prática | Prioridade |
|---|---|---|
| 1 | Nunca reutilizar usernames entre identidades/plataformas | 🔴 Crítico |
| 2 | Nunca reutilizar fotos de perfil entre identidades | 🔴 Crítico |
| 3 | Desativar geolocalização em fotos antes de compartilhar | 🔴 Crítico |
| 4 | Não fazer check-in em tempo real (postar depois) | 🟡 Recomendado |
| 5 | Usar aliases de e-mail para cada cadastro novo | 🟡 Recomendado |
| 6 | Revisar configurações de privacidade de redes sociais mensalmente | 🟡 Recomendado |
| 7 | Não publicar informações que combinadas identifiquem você (cidade+profissão+idade+universidade) | 🔴 Crítico |
| 8 | Usar navegadores/containers diferentes para identidades diferentes | 🟡 Recomendado |
| 9 | Assumir que qualquer canal não E2EE é monitorado | 🔴 Crítico |
| 10 | Manter travel phone/laptop limpo para viagens internacionais | 🟢 Avançado |
...
### 63.1 Controles Parentais com Respeito à Privacidade

| Ferramenta | Open-Source | Plataformas | Tipo | Diferencial | Link |
|---|---|---|---|---|---|
| **NextDNS (perfil Family)** | Parcial | Multi | DNS | Filtros customizáveis por dispositivo, bloqueio de categorias | https://nextdns.io/ |
| **CleanBrowsing (Family)** | ❌ Não | DNS | DNS | DNS 185.228.168.168 — filtra conteúdo adulto e malware automaticamente | https://cleanbrowsing.org/ |
| **OpenDNS FamilyShield** | ❌ Não | DNS | DNS | DNS 208.67.222.123 — filtra conteúdo adulto, da Cisco | https://www.opendns.com/setupguide/#familyshield |
| **Kidslox** | ❌ Não | Android, iOS | App | Controle de tempo de tela, filtro de conteúdo, bloqueio de apps | https://kidslox.com/ |
| **Qustodio** | ❌ Não | Multi | App | Monitoramento multiplataforma, relatórios de atividade | https://www.qustodio.com/ |
| **Google Family Link** | ❌ Não | Android | App | Controle de tempo, aprovação de apps, localização (integrado ao Android) | https://families.google.com/ |
| **Apple Screen Time** | ❌ Não | iOS/macOS | Nativo | Limites de tempo, restrições de conteúdo, relatórios semanais | — |
| **Pi-hole + listas familiares** | ✅ Sim | Auto-hospedado | DNS | Bloqueio customizado em toda a rede com listas específicas para família | https://pi-hole.net/ |

### 63.2 Riscos por Faixa Etária

| Faixa Etária | Riscos Principais | Ações Recomendadas |
|---|---|---|
| **Crianças (< 8)** | Conteúdo impróprio acidental, compras in-app, exposição a anúncios | DNS familiar, perfil restrito, apps curados, sem acesso livre a YouTube |
| **Crianças (8-12)** | Cyberbullying, compartilhamento de informações pessoais, jogos com chat | Conversa sobre privacidade, apps de mensagem familiares (Signal), NewPipe em vez de YouTube |
| **Adolescentes (12-15)** | Redes sociais, sexting, grooming, pressão de pares online, fake news | Configurações de privacidade em redes sociais, educação sobre engenharia social, sem dados pessoais em perfis |
| **Adolescentes (15-17)** | Todos os anteriores + exposição de identidade, pegada digital permanente | Ensinar sobre OPSEC, gerenciador de senhas próprio, educação sobre consequências de posts permanentes |
| **Idosos (> 65)** | Golpes telefônicos, phishing, scams, dificuldade técnica | Brave Browser (simples), DNS seguro no roteador, treinamento anti-phishing, contato de suporte familiar |

### 63.3 Alternativas Seguras para Crianças/Adolescentes

| Serviço Comum | Alternativa Segura | Por quê |
|---|---|---|
| YouTube | **NewPipe / LibreTube** | Sem algoritmo de recomendação, sem rastreamento, sem anúncios |
| Google Maps | **Organic Maps / OsmAnd** | Sem rastreamento de localização |
| WhatsApp | **Signal** | E2EE real, sem coleta de metadados pela Meta |
| Discord (sem supervisão) | **Signal Groups / Element** | Ambientes mais seguros e controlados |
| Chrome | **Firefox Focus / Brave** | Bloqueio nativo de rastreadores |
| Google Search | **DuckDuckGo / Brave Search** | Sem perfil de busca, sem personalização |
| Gmail | **Proton Mail / Tuta** | E2EE, sem leitura de e-mails para anúncios |
| TikTok/Instagram | Restringir tempo + conta privada | Configurar conta como privada, desativar sugestão para não-seguidores |

### 63.4 Checklist de Conversas com Crianças sobre Segurança Digital

| # | Tema | Idade sugerida |
|---|---|---|
| 1 | "Nunca compartilhe seu nome real, escola ou endereço online" | 6+ |
| 2 | "Se alguém que você não conhece te mandar mensagem, mostre para um adulto" | 6+ |
| 3 | "Fotos e vídeos que você envia podem ser compartilhados para sempre" | 10+ |
| 4 | "Senhas são como escovas de dente — não se compartilha" | 8+ |
| 5 | "Nem tudo que você lê na internet é verdade" | 8+ |
| 6 | "Se algo te deixa desconfortável online, fale com um adulto — você não vai ter problemas" | 6+ |
| 7 | "Cuidado com downloads e links — podem ter vírus" | 10+ |
| 8 | "O que você posta hoje pode ser encontrado daqui a 10 anos" | 12+ |
| 9 | "Pessoas online podem mentir sobre quem são" | 10+ |
| 10 | "Cyberbullying é tão sério quanto bullying presencial — denuncie" | 10+ |

> 💡 **Abordagem recomendada:** Segurança digital para crianças deve ser tratada como educação, não como vigilância. O objetivo é empoderar, não controlar. Confiança e diálogo aberto são mais eficazes que monitoramento secreto.
...
### Segurança Física
A segurança digital mais sofisticada falha se o dispositivo físico for comprometido. Criptografia de disco, senhas de BIOS, telas de privacidade e USB data blockers são tão importantes quanto VPNs e firewalls.

### OPSEC é Comportamento
A ferramenta mais segura do mundo não protege quem reutiliza senhas, clica em links suspeitos, usa o mesmo username em tudo, ou posta localização em tempo real. Segurança começa com hábitos.

---

> **Referência:** Este documento é material de apoio para o CyberShield e deve ser mantido atualizado conforme novas ferramentas, auditorias e incidentes sejam publicados. Última revisão: Abril 2026 (v5.0).